...
// and io300_writec, with some bonus unnecessary seeks.
...
int main(int argc, char* argv[]) {
    ...
    struct io300_file* in = io300_open(argv[1], MODE_READ, "\e[0;31min\e[0m");
    if (in == NULL) {
        return 1;
    }

    struct io300_file* out = io300_open(argv[2], MODE_WRITE, "\e[0;32mout\e[0m");
    if (out == NULL) {
        io300_close(in);
        return 1;
    }
    ...
    for (int i = 0; i < filesize; i++) {
        ...
        for (int j = 0; j < 10; j++) {
            if (io300_seek(in, 0) == -1) {
                fprintf(stderr, "error: seek should not fail.\n");
                exit_status = 1;
                break;
            }
            if (io300_seek(in, filesize - 1) == -1) {
                fprintf(stderr, "error: seek should not fail.\n");
                exit_status = 1;
                break;
            }
