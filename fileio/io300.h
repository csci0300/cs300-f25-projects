...
/*
 *   Create a new io300_file struct for the given file
 *   Arguments:
 *    - path:  Path to the file
 *    - mode:  File mode (see below)
 *    - description:  Description of the file (for debugging)
 *
 *    The file mode must be one of the following:
 *    - MODE_READ:  open file as read-only
 *    - MODE_WRITE: open file for reading and writing.  If the file exists,
 *      it is truncated to zero bytes; if the file does not exist, it is created.
 *    - MODE_RDWR:  open the file for reading and writing
 */
struct io300_file* io300_open(const char* path, int mode, char* description);
 ...
 *  Return -1 (EOF) on failure or if the end of the file has been reached.
 *  Note that this behavior emulates `fgetc` from the C standard library
 */
